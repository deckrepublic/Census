 ...
 */
public class CensusReducerQ1 extends Reducer<Text, BookRecord, Text, IntWritable> {
...
//should only have one value in values due to combiner merging all records for each state, should be 50 keys
    ...
    protected void reduce(Text key, Iterable<BookRecord> values, Context context) throws IOException, InterruptedException {
		...
		combine(key,values);
		double numberOfHomes = 0;
		IntWritable [] occupied = theRecord.getData().getTenure().getOccupied(); //get the array of homes owned and rented

		double ownedHomes = occupied[0].get(); //number of homes owned
		double rentedHomes = occupied[1].get(); //number of homes rented
		numberOfHomes = ownedHomes + rentedHomes;

		int ownPercent = (int) Math.round((ownedHomes/numberOfHomes) * 100); //now get the percentage in int form
		int rentPercent = (int) Math.round((rentedHomes/numberOfHomes) * 100);

		//get some text to write
		Text owner = new Text(key.toString() + " percentage owned: %");
		Text renter = new Text(key.toString() + " percentage rented: %");
		//have to write both percentages
		context.write(owner, new IntWritable(ownPercent));
		context.write(renter, new IntWritable(rentPercent));
